...
import "../lib/dn404/src/DN404.sol";
...
contract Koala is DN404, OwnableRoles {
    ...
    event NFTTransfer(address from, address to, uint256 id);
    ...
    function tokenURI(uint256 id) public view returns (string memory result) {
        return _tokenURI(id);
    }

    /// @dev Hook that is called after any NFT token transfers, including minting and burning.Waiting to be perfected
    function _afterNFTTransfer(address from, address to, uint256 id) internal override {
        emit NFTTransfer(from, to, id);
    }

    function _tokenURI(uint256 id) internal override view returns (string memory){
        if (!_exists(id)) revert TokenDoesNotExist();
        if (bytes(_baseURI).length != 0) {
            string memory result = LibString.replace(_baseURI, "{id}", LibString.toString(id));
        }
        else{
            return "ipfs://QmaS4YNkPTZo3BkvBHsGb3boiFVs3viHUmucFaCMHRooaB";
        }
    }
